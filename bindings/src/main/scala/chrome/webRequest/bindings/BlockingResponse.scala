...
object BlockingResponse {
  def apply(cancel: js.UndefOr[Boolean] = js.undefined,
            ...
            requestHeaders: js.UndefOr[js.Array[HttpHeader]] = js.undefined): BlockingResponse = {
    js.Dynamic.literal(
      cancel = cancel,
...
trait BlockingResponse extends js.Object {
  ...
  /**
    ...
    * response headers instead. Only return responseHeaders if you really want to modify the headers in order to limit
    * the number of conflicts (only one extension may modify responseHeaders for each request).
    */
  ...
  /**
    * Optional:
    * Only used as a response to the onBeforeSendHeaders event. If set, the request is made with these request headers
    * instead.
    */
